...
def sync_same_node(node, repo_name):
    ...
    if flag:
        ...
        build_image(parameter)
        commits = my_repo.get_commits(sha=my_repo.default_branch)[0]
        node["commit_id"] = commits.sha                                     # storing the commit sha of the node.
        for ids in node["children"]:
            dfs_same_node(ids)
        records.update_one({"_id": node["_id"]},
                           {"$set": {"last_updated_time": str(datetime.now().replace(microsecond=0))}})
...
def dfs_same_node(node_id):             # recursively re-building the subtree on update of the parent node
    child_node = records.find_one({"_id": node_id})
    if child_node["component_name"]:
        parameter = {"repo_name": child_node["repo_name"], "component_name": child_node["component_name"], "tag": child_node["tag"]}
        build_image(parameter)

        for ids_child in child_node["children"]:
            dfs_same_node(ids_child)
    else:
        parameter = {"dockerfile_local_path": child_node["dockerfile_local_path"],
                     "img_name": child_node["img_name"],
                     "tag": child_node["tag"]}
    ...
    records.update_one(
        {"_id": child_node["_id"]}, {"$set": {"last_updated_time": str(datetime.now().replace(microsecond=0)), "last_synced_time": str(datetime.now().replace(microsecond=0))}}
    )
    # records.update_one(
    #     {"_id": child_node["_id"]}, {"$set": {"last_synced_time": str(datetime.now().replace(microsecond=0))}}
    # )
...
def force_build(node_id):
    ...
    build_image(parameter)
    # To be confirmed, whether to rebuild the child nodes too or not!!
    for ids in node["children"]:
        dfs_same_node(ids)
